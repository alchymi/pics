...
if (is_dir($dir)) {
  foreach (scandir($dir) as $f) {
    ...
    if (is_file($path) && preg_match('~\.(jpe?g|png|gif|webp)$~i', $f)) {
      // Récup dimensions pour réserver l'espace
      $w = $h = 0;
      if ($info = @getimagesize($path)) { $w = $info[0] ?? 0; $h = $info[1] ?? 0; }
      $files[] = [
        'name' => $f,
        'url'  => $base . '/' . $f,
        'ts'   => filemtime($path) ?: 0,
...
<body class="bg-zinc-950 text-zinc-100">
  ...
  <main class="mx-auto max-w-7xl px-4 py-6">
    ...
    <div class="masonry columns-1 sm:columns-2 md:columns-3 lg:columns-4">
      <?php foreach ($files as $img): ?>
        <?php
          $w = max(1, (int)$img['w']); $h = max(1, (int)$img['h']);
          $ratioStyle = "aspect-ratio: {$w} / {$h};";
        ?>
        <figure class="masonry-item group overflow-hidden rounded-xl bg-zinc-900 border border-zinc-800">
          <img
            ...
            decoding="async"
            width="<?= $w ?>" height="<?= $h ?>"
            style="<?= $ratioStyle ?> background:#0b0b0f;"
            class="transition-transform duration-300 ease-out group-hover:scale-[1.02] cursor-zoom-in"
          ...
          <figcaption class="px-3 py-2 text-xs text-zinc-400 flex items-center justify-between">
            ...
            <button
              ...
              title="Copier l’URL">Copy</button>
          </figcaption>
        </figure>
      <?php endforeach; ?>
    </div>
  ...
  <script>
    ...
    function copyToClipboard(text) { navigator.clipboard.writeText(text).then(()=>alert('URL copiée')).catch(()=>{}); }
  </script>
</body>
</html>